...
func NewGobuilds(ctx context.Context, workingDirectory string, buildConfigs map[string]Config, imageConfigs map[string]ImageConfig, opts ...Option) (Interface, error) {
	if workingDirectory == "" {
		workingDirectory = "."
	}
	...
	for importpath, buildConfig := range buildConfigs {
		...
		g.builders[importpath] = builderWithConfig{
			builder:     builder,
			config:      buildConfig,
			imageConfig: imageConfigs[importpath],
		}
	}
	return g, nil
}
